...
assets_folder = os.path.abspath(    os.path.join(os.path.dirname(__file__), "..", "assets/")).replace("\\", "/")
...
class GameScreen(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.hit_button = tk.Button(
            self.bottom_frame, text="Hit", width=25, command=self.hit
        )
        self.stick_button = tk.Button(
            self.bottom_frame, text="Stick", width=25, command=self.stick
        )

        self.play_again_button = tk.Button(
            self.bottom_frame, text="Play Again", width=25, command=self.play_again
        )
        self.quit_button = tk.Button(
            self.bottom_frame, text="Quit", width=25, command=self.destroy
        )
    ...
    def display_table(self, hide_dealer=True, table_state=None):
        ...
        for card_number, card_image in enumerate(player_card_images):
            self.game_screen.create_image(
                (
                    self.CARD_ORIGINAL_POSITION + self.CARD_WIDTH_OFFSET * card_number,
        ...
        if table_state["has_winner"]:
            ...
            if table_state["has_winner"] == "p":
                self.game_screen.create_text(
                    self.WINNER_TEXT_COORDS, text="YOU WIN!", font=(None, 50)
                )
            elif table_state["has_winner"] == "dp":
                self.game_screen.create_text(
                    self.WINNER_TEXT_COORDS, text="TIE!", font=(None, 50)
                )
            else:
                self.game_screen.create_text(
                    self.WINNER_TEXT_COORDS, text="DEALER WINS!", font=(None, 50)
                )
